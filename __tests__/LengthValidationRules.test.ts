...
import { restrictMaximumLength, restrictMinimumLength } from '@try-at-software/input-elements';
import each from 'jest-each';

describe('LengthValidationRules', (): void => {
    each([getRandomString(), null, '', undefined]).it('Should restrict minimum length with some message', (message: string): void => {
    ...
    each([getRandomString(), null, '', undefined]).it('Should restrict maximum length with some message', (message: string): void => {
        // Generate a random length to test the minimum length validation rule.
        const length = getRandomNumber(15, 20);
